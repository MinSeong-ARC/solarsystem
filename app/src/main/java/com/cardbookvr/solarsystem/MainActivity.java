...
public class MainActivity extends CardboardActivity implements IRenderBox {
    ...
    float EDAY_RATE = 10f;          // animation rate for one earth rotation (seconds per rotation)
    float DEG_PER_EHOUR = (360f / 24f / EDAY_RATE); // rotation scale factor e.g. to animate earth: dt * 24 * DEG_PER_EHOUR
    float EYEAR_RATE = 1500f;       // animation rate for one earth rotation (seconds per orbit) (real is EDAY_RATE * 365.26)
    float DEG_PER_EYEAR = (360f / EYEAR_RATE); // orbit scale factor
